...
@login_required(login_url='doctorlogin')
@user_passes_test(is_doctor)
def doctor_update_patient_status_view(request, pk):
    patient = models.Patient.objects.get(id=pk, assignedDoctorId=request.user.id)
    if request.method == 'POST':
        form = forms.PatientStatusUpdateForm(request.POST, instance=patient)
        if form.is_valid():
            form.save()
            return redirect('doctor-view-patient')
    else:
        form = forms.PatientStatusUpdateForm(instance=patient)
    doctor = models.Doctor.objects.get(user_id=request.user.id)
    return render(request, 'hospital/doctor_update_patient_status.html', {'form': form, 'doctor': doctor, 'patient': patient})
...
def doctor_view_discharge_patient_view(request):
    ...
    doctor = models.Doctor.objects.get(user_id=request.user.id)
    dischargedpatients = models.PatientDischargeDetails.objects.all().distinct().filter(patientId__in=models.Patient.objects.filter(assignedDoctorId=request.user.id).values_list('id', flat=True))
